...
rimraf('./designDist', function (err) {
    if(err){
        console.log(err);
    }
});
function getExports(project){
    ...
    config.apps.forEach((app)=>{
        entry[`${app}/app`] = [`${config.designMainDir}/${project}/${app}/main.js`,'./dev-client.js']
        ...
        rules.push(
            {
                ...
                exclude:[path.resolve('./designDist'),/node_modules/]//排除解析dist文件夹
        ...
        cacheGroups[`${app}Assets`] = {
            ...
            test: path.resolve(`${config.designMainDir}/${project}/${app}/assets`),
            // test: /assets/,
            name: `${app}/assets`,
            priority: 10,
            enforce: true,
        ...
        alias[`@${app}`] = path.resolve(`${config.designMainDir}/${project}/${app}`)
        plugins.push(new HtmlWebpackPlugin({
            filename: `${app}/index.html`,//真正输出的地址output.path+filename=./dist/index.html
            template:`${config.designMainDir}/${project}/${app}/index.html`,//INdex的模板
            inject: true,
            hash:true,
            title:app,
            minify: {
    ...
    return {
        ...
        output:{
            path:path.resolve(__dirname,'designDist',project),
        ...
        resolve: {
            ...
            alias:Object.assign(alias,{
                '@parent':path.resolve(config.parentDesignDir),
            ...
            rules:rules.concat([
                ...
                },
                {
                    ...
                    use:{
                        ...
                        options:{
                            mainDir:config.designMainDir.replace('./','')
                        }
                    },
                    exclude:[path.resolve('./designDist'),/node_modules/],
                    //exclude: file => (
                    //    /node_modules/.test(file) && !/\.vue\.js/.test(file)
                    //),
                    include:[path.resolve(`${config.designMainDir}`),path.resolve(`${config.parentDesignDir}`)]
                },
                {
                    test:/\.(html|htm)/,
                ...
                },
                {
                    ...
                    include: [path.resolve(`${config.designMainDir}/${project}`),path.resolve(`${config.parentDesignDir}`)]//只编译src文件夹 但是node_modules除外
                },
                {
                    test: /\.css$/,
                    //loader:'style-loader!css-loader'
        ...
        plugins:plugins.concat([
            ...
            new CleanWebpackPlugin(['./designDist/'+project]),//删除文件夹插件
            ...
            new NotFoudEntryPlugin({
                mainDir:config.designMainDir,
                parentDir:config.parentDesignDir
            }),
